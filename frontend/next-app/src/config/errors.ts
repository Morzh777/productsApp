 ...
 * для создания и обработки различных типов ошибок.
 */
...
export const ERROR_MESSAGES = {
  ...
  INVALID_PRODUCTS_FORMAT: 'Неверный формат данных товаров',
  INVALID_PRODUCT_FORMAT: 'Неверный формат данных товара',
  INVALID_CATEGORIES_FORMAT: 'Неверный формат категорий',
  INVALID_UPDATED_PRODUCT_FORMAT: 'Неверный формат данных обновленного товара',
  INVALID_CREATED_PRODUCT_FORMAT: 'Неверный формат данных созданного товара',
  
  // Сеть
  NETWORK_ERROR: 'Ошибка сети',
  TIMEOUT: 'Превышено время ожидания',
  CONNECTION_FAILED: 'Не удалось подключиться к серверу',
  INVALID_JSON_RESPONSE: 'Ответ сервера не содержит JSON',
...
export class ErrorHandler {
  static validationError(message: string = ERROR_MESSAGES.VALIDATION_FAILED, details?: unknown): AppError {
    return new AppError(ErrorType.VALIDATION, message, 'VALIDATION_ERROR', details);
  }

  static networkError(message: string = ERROR_MESSAGES.NETWORK_ERROR, details?: unknown): AppError {
    return new AppError(ErrorType.NETWORK, message, 'NETWORK_ERROR', details);
  }

  static notFoundError(message: string = ERROR_MESSAGES.PRODUCT_NOT_FOUND, details?: unknown): AppError {
    return new AppError(ErrorType.NOT_FOUND, message, 'NOT_FOUND', details);
  }

  static serverError(message: string = ERROR_MESSAGES.SERVER_ERROR, details?: unknown): AppError {
  ...
  static handleHttpError(status: number, message?: string): AppError {
    switch (status) {
      ...
      case HTTP_ERROR_CODES.SERVICE_UNAVAILABLE:
        return new AppError(ErrorType.SERVER, ERROR_MESSAGES.SERVICE_UNAVAILABLE, 'SERVICE_UNAVAILABLE');
      default:
  ...
  static handleUnknownError(error: unknown): AppError {
    ...
    if (error instanceof Error) {
      return new AppError(ErrorType.UNKNOWN, error.message, 'UNKNOWN_ERROR', error);
    }

    return new AppError(ErrorType.UNKNOWN, ERROR_MESSAGES.UNKNOWN_ERROR, 'UNKNOWN_ERROR', error);
  }
  ...
  static logError(error: AppError, context?: string): void {
    ...
    if (process.env.NODE_ENV === 'production' && error.type === ErrorType.UNKNOWN) {
      return;
    }
